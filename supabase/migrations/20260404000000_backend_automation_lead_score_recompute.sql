-- Backend Automation: Nightly Lead-Score Recompute
--
-- Creates recalculate_all_contact_scores_system() which mirrors the
-- client-side buildContactUrgency() logic in src/lib/contactUrgency.ts.
-- It recalculates the urgency tier (category column) for every contact
-- based on overdue tasks, due-today tasks, due-soon tasks, sequence actions,
-- upcoming appointments, and inactivity.
--
-- Score thresholds: immediate >= 130, priority >= 80, planned >= 35, backlog < 35
--
...
BEGIN
  ...
  LOOP
    ...
    -- Overdue tasks (past due, not today, not completed)
    SELECT COUNT(*)
    INTO v_overdue
    ...
    -- Tasks due within 48 hours (but not today or overdue)
    SELECT COUNT(*)
    INTO v_due_soon
    FROM public.contact_tasks t
    WHERE t.contact_id = v_contact.id
    ...
    IF v_due_soon > 0 THEN
      v_score := v_score + 45 + LEAST(v_due_soon * 5, 15);
    END IF;

    -- Nurture sequence actions due within 24 hours
    SELECT COUNT(*)
    INTO v_sequence_due
    ...
    -- Upcoming appointments within 24h
    SELECT EXISTS (
    ...
    IF v_appt_within_24h THEN
      v_score := v_score + 55;
    ELSE
      -- Appointments within 72h (only if not already within 24h)
      SELECT EXISTS (
        SELECT 1 FROM public.appointments a
        WHERE a.contact_id = v_contact.id
          AND a.start_time > now() + INTERVAL '24 hours'
