-- Improve nurture step completion:
-- - When completing a step, immediately create the next task/prompt contact_task + step_run (if next step is task/prompt),
--   so contact-level UI shows the next day even before it is due.
-- - Add notifications for steps that become due immediately (next_step_at <= now()).
-- - Respect cadence pausing for notifications (runner still handles due notifications).

-- Prevent duplicate notifications for the same step_run.
...
BEGIN
  ...
  -- No next step: end the enrollment.
  ...
  -- If next step is a task/prompt, create its contact task and step_run immediately (even if due_at is in the future).
  IF v_next_step_type IN ('task', 'prompt') THEN
    ...
    ELSIF v_next_task_id IS NULL THEN
      ...
      SET task_id = v_created_task_id,
          ...
          activated_at = now(),
          updated_at = now()
      WHERE id = v_next_step_run_id;
    END IF;

    -- Create in-app notification immediately only when the cadence is unpaused and the step is already due.
