-- Log touches via SECURITY DEFINER RPC (reliable permissions + clearer errors for clients
-- where direct PostgREST insert to touches fails). Harden complete_nurture_step_and_advance:
-- resolve enrollment from the step run (fixes stale task.sequence_enrollment_id) and allow
-- the linked task assignee to complete when they can access the contact.
...
COMMENT ON FUNCTION public.log_touch_manual(UUID, TEXT, TEXT, TIMESTAMPTZ) IS
  'Insert a touch for the current user when they can access the contact; bypasses touches RLS edge cases.';
...
BEGIN
  ...
  IF v_run.id IS NULL THEN
    RAISE EXCEPTION 'Step run not found';
  END IF;

  -- p_enrollment_id is accepted for API compatibility; enrollment is always resolved from the step run.

  SELECT *
  INTO v_enrollment
  FROM public.nurture_sequence_enrollments e
  WHERE e.id = v_run.enrollment_id
  ...
  IF NOT public.user_can_access_contact(v_enrollment.contact_id) THEN
  ...
  IF v_enrollment.user_id IS DISTINCT FROM auth.uid() THEN
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM public.contact_tasks ct
      WHERE ct.id = v_run.task_id
        AND ct.user_id = auth.uid()
        AND ct.contact_id = v_enrollment.contact_id
    ) THEN
      RAISE EXCEPTION 'Enrollment not found or not accessible';
    END IF;
  END IF;

  IF v_run.step_index IS DISTINCT FROM v_enrollment.current_step_index THEN
  ...
  IF v_next_step_type IN ('task', 'prompt') THEN
    ...
    IF v_next_step_run_id IS NULL THEN
      ...
      VALUES (v_enrollment.contact_id, v_enrollment.user_id, v_next_title, v_next_body, v_next_at, v_enrollment.id)
