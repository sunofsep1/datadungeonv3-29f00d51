...
/**
 * Instant light acknowledgement to the PROSPECT. Keeps speed-to-lead without
 * automating the sales conversation — identifies the sender + agency (Spam Act)
 * and STOP is appended by appendCommercialOptOutIfMissing.
 */
export function composeProspectAckSms(ctx: SellerLeadContext): string {
  const first = ctx.firstName || "there";
  const base =
    `Hi ${first}, thanks for requesting a property appraisal — it's Greg Leigh from ` +
    `Queensland Sotheby's International Realty. I've got your details and will be in touch personally very shortly.`;
