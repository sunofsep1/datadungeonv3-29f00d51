...
export function ListingContactLinksPanel({ listingId }: Props) {
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
          {links.map((link) => {
            ...
            return (
              <li key={link.id} className="flex items-center justify-between gap-2 rounded-lg border border-border/70 p-2.5">
                <div className="min-w-0">
                  <Link to={`/contacts/${link.contact_id}`} className="text-sm font-medium text-primary hover:underline truncate block">
                    {name}
                  </Link>
                  <p className="text-xs text-muted-foreground capitalize">{roleLabel.replace(/_/g, " ")}</p>
                </div>
                <Button type="button" variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-destructive" onClick={() => void deleteLink.mutateAsync({ id: link.id, listingId })}>
