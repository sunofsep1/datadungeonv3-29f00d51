...
export type ContactSmartListId = "all" | ContactClassificationKey | "stale" | "no_next_touch";
...
export function parseSmartListParam(raw: string | null): ContactSmartListId | null {
  ...
  if (!raw) return null;
  if (raw === "stale" || raw === "no_next_touch") return raw;
...
export function isClassificationSmartList(id: ContactSmartListId): id is ContactClassificationKey {
  return id !== "all" && id !== "stale" && id !== "no_next_touch";
}
